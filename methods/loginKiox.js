let Empleado = "";
import { firebase, db, auth ,EXsignInWithEmailAndPassword,EXcollection,EXgetDocs,EXquery, EXwhere} from './conexiones.js';
...
$(".btnIngresar").click(function(){
    ...
    if (correo != "" && password != "") {
        var url="/Operatives";
        EXsignInWithEmailAndPassword(auth,correo, password).then(usuario =>{                    
            const Empleado = EXquery(EXcollection(db, url), EXwhere("UserId", "==", usuario.user.uid));
            ...
            // collection(db,url).where("UserId","==",usuario.user.uid).get().then(data =>{
